    ...
    for (auto _ : state) {
        ...
        auto producer = [&]() {
            ...
            for (int i = 0; i < total / num_producers; ++i)
                while (!q.try_push(i));
            produced.fetch_add(total / num_producers);
        };
...
static void BM_Throughput_1P1C(benchmark::State& s) { run_throughput(s, 1, 1); }
static void BM_Throughput_2P2C(benchmark::State& s) { run_throughput(s, 2, 2); }
static void BM_Throughput_4P4C(benchmark::State& s) { run_throughput(s, 4, 4); }
...
BENCHMARK(BM_Throughput_1P1C)->UseRealTime();
BENCHMARK(BM_Throughput_2P2C)->UseRealTime();
BENCHMARK(BM_Throughput_4P4C)->UseRealTime();
BENCHMARK(BM_Latency_RTT);
